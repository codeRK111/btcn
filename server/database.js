...
exports.makeWithdrawal = function(userId, satoshis, withdrawalAddress, withdrawalId, callback) {
    ...
    getClient(function(client, callback) {
        ...
        client.query("UPDATE users SET balance_satoshis = balance_satoshis - $1 WHERE id = $2",
            ...
            client.query('INSERT INTO fundings(user_id, amount, litecoin_withdrawal_address, withdrawal_id) ' +
    ...
    query("SELECT * FROM fundings WHERE user_id = $1 AND amount < 0 ORDER BY created DESC", [userId], function(err, result) {
        ...
        var data = result.rows.map(function(row) {
           return {
               ...
               destination: row.litecoin_withdrawal_address,
               status: row.litecoin_withdrawal_txid,
               created: row.created
           };
        });
        callback(null, data);
...
exports.getDeposits = function(userId, callback) {
    ...
    query("SELECT * FROM fundings WHERE user_id = $1 AND amount > 0 ORDER BY created DESC", [userId], function(err, result) {
        ...
        var data = result.rows.map(function(row) {
            return {
                amount: row.amount,
                txid: row.litecoin_deposit_txid,
                created: row.created
            };
        });
        callback(null, data);
...
exports.setFundingsWithdrawalTxid = function(fundingId, txid, callback) {
    ...
    query('UPDATE fundings SET litecoin_withdrawal_txid = $1 WHERE id = $2', [txid, fundingId],
        function(err, result) {
           if (err) return callback(err);
...
exports.getSiteStats = function(callback) {
    ...
    var tasks = [
        ...
        function(callback) {
            query('SELECT COUNT(*) FROM fundings WHERE amount < 0 AND litecoin_withdrawal_txid IS NULL', as('pending_withdrawals', callback));
        },
        function(callback) {
...
exports.makeTransfer = function(uid, fromUserId, toUsername, satoshis, callback){
    ...
    getClient(function(client, callback) {
        async.waterfall([
            function(callback) {
                client.query("UPDATE users SET balance_satoshis = balance_satoshis - $1 WHERE id = $2",
                  [satoshis+1000, fromUserId], callback)
            },

...
CREATE TABLE fundings (
    ...
    litecoin_withdrawal_txid text,
    litecoin_withdrawal_address text,
    created timestamp with time zone DEFAULT now() NOT NULL,
    description text,
    litecoin_deposit_txid text,
    withdrawal_id UUID,
    CONSTRAINT fundings_withdrawal_id_key UNIQUE (withdrawal_id)
);

ALTER TABLE ONLY fundings
    ADD CONSTRAINT fundings_user_id_litecoin_deposit_txid_key UNIQUE (user_id, litecoin_deposit_txid);

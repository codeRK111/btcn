 ...
 *
 * 1.- Connect with socket.io to ltcnyan.com
...
Chat.prototype.onSay = function(socket, message, customChannelName,isBot) {
    if (!socket.user)
    ...
    if(customChannelName)
        if(isChannelNameInvalid(customChannelName))
            return sendError(socket, '[say] invalid channel name: '+customChannelName+':'+isBot+':'+socket);
        else

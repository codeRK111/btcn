...
var litecoinjs = require('litecore-lib');
...
exports.security = function(req, res) {
    ...
    if (!user.mfa_secret) {
        ...
        var qrUri = 'otpauth://totp/ltcnyan:' + user.username + '?secret=' + user.mfa_potential_secret + '&issuer=ltcnyan';
...
exports.enableMfa = function(req, res, next) {
    ...
    if (otp !== expected) {
        ...
        var qrUri = 'otpauth://totp/ltcnyan:' + user.username + '?secret=' + secret + '&issuer=ltcnyan';
        user.qr_svg = qr.imageSync(qrUri, {type: 'svg'});
        user.sig = sig;
...
exports.handleWithdrawRequest = function(req, res, next) {
    ...
    try {
        ...
        if (destination.length<=10)
            return res.render('withdraw-request', { user: user,  id: uuid.v4(), warning: 'Destination address is not a litecoin one' });
    } catch(ex) {
